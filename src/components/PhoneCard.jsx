    ...
    <div>
      <div className="card bg-base-100 shadow-sm grow">
        <figure className="px-8 py-8 w-full h-[300px]">
          <img className="w-full h-full cover" src={phone.img} alt="Shoes" />
        </figure>
        <div className="card-body grow ">
          <h2 className="card-title">{phone.phone_name}</h2>
          <p className="text-start grow">{phone.description}</p>
        ...
        <div>
       {deletable && <button className="absolute -right-1 -top-2 cursor-pointer" onClick={()=>{handleDelete(phone.id)}}><FaDeleteLeft size={20} /></button>}
        </div>
      </div>
    </div>
  );
